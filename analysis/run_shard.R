...
fit_raen_mm <- function(X, y, gamma    = 2,
  ...
  for (irls in seq_len(max_irls)) {
    ...
    for (cd_iter in seq_len(max_cd)) {
      beta_old <- beta
      ## 2026-09-01 CORRECTION. lambda_fixed comes from cv.glmnet, which
      ## minimises 1/(2n)||y-Xb||^2 + lambda[...]. Written against the
      ## 1/2||.||^2 loss that cd_sweep implements, the penalty is n*lambda.
      ## Passing lambda_fixed unscaled penalised the fit by a factor of n
      ## too weakly (n = 356), giving ~830 selected genes where the
      ## corrected scaling gives ~45, with test RMSE unchanged (p = .968).
      ## Verified against S6_verify_patch.R. Pre-patch copy of this file is
      ## run_shard_PRE_PATCH_2026-09-01.R.
